    ...
    <CardContent className="p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {loading ? <Skeleton className="h-8 w-24 mt-1" /> : <p className="text-2xl font-bold font-display mt-1">{value}</p>}
        </div>
    ...
    <>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Revenue" value={`$${stats?.totalRevenue?.toLocaleString() || 0}`} icon={DollarSign} loading={loading} />
          <StatCard title="Total Orders" value={`${stats?.totalOrders || 0}`} icon={ShoppingCart} loading={loading} />
                  ...
                  {stats?.recentOrders.map((o) => (
                    <div key={o.id} className="flex items-center justify-between py-2 border-b last:border-0">
                      ...
                      <div className="flex items-center gap-3">
                        <Badge className={statusColor[o.status] || "bg-secondary text-secondary-foreground"}>
                          {o.status}
                        </Badge>
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <p className="text-sm text-muted-foreground py-4">All products are in stock!</p>
              ) : (
                <div className="space-y-3">
