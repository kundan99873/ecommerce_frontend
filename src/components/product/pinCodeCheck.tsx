...
const SERVICEABLE_PINS = [
  ...
  "90001", "90002", "90003", "90004", "90005", "90210",
...
interface PincodeCheckProps {
  onResult?: (result: { deliverable: boolean; pincode: string; eta?: string }) => void;
  compact?: boolean;
}
...
const PincodeCheck = ({ onResult, compact = false }: PincodeCheckProps) => {
  ...
  const [result, setResult] = useState<{ deliverable: boolean; eta?: string } | null>(null);
  ...
  return (
    <div className={compact ? "space-y-2" : "space-y-3"}>
      <div className="flex items-center gap-2">
        ...
        <span className={`font-medium ${compact ? "text-xs" : "text-sm"}`}>Delivery Check</span>
      </div>
      <div className="flex gap-2">
        ...
        <Button
          ...
          onClick={handleCheck}
          disabled={pincode.length !== 5 || checking}
          className={compact ? "text-xs h-8" : "text-sm"}
        >
      ...
      {result && (
        <div className={`flex items-start gap-2 rounded-lg px-3 py-2 text-sm ${
          result.deliverable ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"
        }`}>
          {result.deliverable ? (
            <>
            ...
            <>
              ...
              <p className="font-medium text-xs">Not serviceable at this PIN code</p>
            </>
          )}
        </div>
      )}
